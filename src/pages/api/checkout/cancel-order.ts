...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    existingData.cancelledAtStep = existingData.currentStep ?? 0;
      ...
      `${baseUrl}/en/jsonapi/commerce_order/default/${orderUuid}`,
      {
        ...
        body: JSON.stringify({
          data: {
            ...
            attributes: {
              state: 'canceled',
              field_checkout_data: JSON.stringify(existingData),
            },
          },
        }),
      },
    ...
    if (!patchRes.ok) {
      ...
      console.error(`[cancel-order] PATCH failed (${patchRes.status}): ${errText.slice(0, 300)}`);
      return new Response(
